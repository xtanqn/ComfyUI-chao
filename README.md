# comfyui-chao / ComfyUI Chao Nodes
...
- 提供13个实用节点，涵盖随机数生成、分辨率控制、提示词管理、图像效果等功能
- 复制到 ComfyUI 的 `custom_nodes` 目录并重启后即可使用，搜索 `chao` 或 `ComfyUI-chao` 即可找到

## 安装 / Installation
1. 打开终端，进入 ComfyUI/custom_nodes 目录
2. 克隆仓库：`git clone https://github.com/xtanqn/comfyui-chao.git`
3. 重启 ComfyUI

## 使用指南

### 节点列表
所有节点都位于 **🍡ComfyUI-chao** 类别下：
...
- **可选输入**: 对齐到8倍数（开关）
- **输出**: Latent、宽度、高度
- **特色**: 支持比例筛选分辨率，可选择是否对齐到8的倍数，批量张数默认值为1，确保符合Stable Diffusion要求
...
- **特色**: 提供多种模糊类型和混合方式，可调节辉光强度和范围

#### 11. 4宫格拆分-chao
- **功能**: 将一张4宫格图片拆分成4张独立图片
- **主要输入**: 4宫格图片（必需）
- **可选输入**: 宫格中间的边框宽度、宫格图外围的边框宽度
- **输出**: 分镜批次 (Batch 4) - 包含4张拆分后图片的批次
- **特色**: 支持自定义边框宽度，自动计算拆分尺寸
